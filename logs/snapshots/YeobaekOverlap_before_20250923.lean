...
variable {𝓩 : Type _} [MeasurableSpace 𝓩]
variable {Boundary : Type _} [MeasurableSpace Boundary]
...
structure YeobaekOverlapHypotheses where
  ...
  kernel_lower_bound : ∃ τ_min : ℝ≥0∞, 0 < τ_min ∧ ∀ x, τ_min ≤ ∫⁻ y, K x y ∂μ
...
structure YeobaekProjectionHypotheses
    ...
    (μ : Measure α) (Π : α → α) : Type _ where
  measurable : Measurable Π
  image_measurable : MeasurableSet (Π '' Set.univ)
  finite_univ : μ Set.univ < ⊤
  image_lt_univ : μ (Π '' Set.univ) < μ Set.univ
  projection_agrees : ∀ z, Π (layer.embedInternal z) = layer.projectionCR z
  boundary_stable : ∀ b, Π (layer.embedBoundary b) = layer.embedBoundary b
...
lemma margin_residual_positive
    {Π : α → α}
    (hΠ : YeobaekProjectionHypotheses layer μ Π) :
    μ (Set.univ \ Π '' Set.univ) > 0 := by
  classical
  have hcompl := measure_add_measure_compl (Π '' Set.univ)
  -- If the residual measure were zero, the image would have full measure, contradicting `image_lt_univ`.
  have hset : (Π '' Set.univ)ᶜ = Set.univ \ Π '' Set.univ := by
    ext x; by_cases hx : x ∈ Π '' Set.univ <;> simp [Set.mem_compl, hx]
  have hpos_ne : μ ((Π '' Set.univ)ᶜ) ≠ 0 := by
    intro hzero
    have hEq : μ (Π '' Set.univ) = μ (Set.univ : Set α) := by
      have := hcompl
      simpa [hzero, add_zero] using this
    exact (ne_of_lt hΠ.image_lt_univ) hEq
  -- convert `≠ 0` to strict positivity and rewrite the set as a set difference
  have hpos : 0 < μ ((Π '' Set.univ)ᶜ) :=
...
lemma margin_positive_measure
    ...
    {Π : α → α}
    (hΠ : YeobaekProjectionHypotheses layer μ Π) :
    μ (Set.univ \ Π '' Set.univ) > 0 :=
  margin_residual_positive (layer := layer) (μ := μ) (Π := Π) hΠ
...
theorem yeobaek_margin_exists
    ...
    {Π : α → α}
    (hΠ : YeobaekProjectionHypotheses layer μ Π) :
    (Set.univ \ Π '' Set.univ).Nonempty := by
  classical
  have hpos := margin_positive_measure (layer := layer) (μ := μ) (K := K) (Π := Π) hK hΠ
  have hpos_ne : μ (Set.univ \ Π '' Set.univ) ≠ 0 := ne_of_gt hpos
  by_contra hempty
  have hset : Set.univ \ Π '' Set.univ = (∅ : Set α) := by
    ext x; constructor
    · intro hx
      exact (hempty ⟨x, hx⟩).elim
    · intro hx; simpa using hx
  have hzero : μ (Set.univ \ Π '' Set.univ) = 0 := by
...
theorem kernel_projection_margin_lower_bound_core
  ...
  (h_margin_lower : tau_margin μ Π A B ≥ ε)
  (h_integral_lower : τ_min ≤ ∫⁻ x, ∫⁻ y, K x y ∂μ ∂μ) :
  ∃ c : ℝ≥0∞, c > 0 ∧
    tau_margin μ Π A B ≥ c * (∫⁻ x, ∫⁻ y, K x y ∂μ ∂μ) / (μ A + μ B + 1) := by
  classical
  set integralK := ∫⁻ x, ∫⁻ y, K x y ∂μ ∂μ
  ...
  refine ⟨c, hc_pos, ?_⟩
  have h_margin_lower' : tau_margin μ Π A B ≥ ε := h_margin_lower
...
lemma tau_margin_residual_univ
    {Π : α → α}
    (hΠ : YeobaekProjectionHypotheses layer μ Π) :
    tau_margin μ Π (Set.univ \ Π '' Set.univ) Set.univ = μ (Set.univ \ Π '' Set.univ) := by
  classical
  simp [tau_margin]
...
lemma tau_margin_lower_of_residual
    ...
    (hΠ : YeobaekProjectionHypotheses layer μ Π)
    {A B : Set α} (hA : MeasurableSet A) (hB : MeasurableSet B)
    (h_residual_subset : Set.univ \ Π '' Set.univ ⊆ A)
    (h_image_subset : Π '' A ⊆ B) :
    μ (Set.univ \ Π '' Set.univ) ≤ tau_margin μ Π A B := by
  classical
  let residual := Set.univ \ Π '' Set.univ
  have hsubset : residual ⊆ A ∩ Π ⁻¹' B := by
    intro x hx
    have hxA : x ∈ A := h_residual_subset hx
    have hxB : Π x ∈ B := by
      have hxImage : Π x ∈ Π '' A := by exact ⟨x, hxA, rfl⟩
...
lemma kernel_integral_lower_bound_global
    ...
    ∃ τ_min : ℝ≥0∞, 0 < τ_min ∧ τ_min ≤ ∫⁻ x, ∫⁻ y, K x y ∂μ ∂μ := by
  classical
  rcases hK.kernel_lower_bound with ⟨τ_base, hτ_pos, hτ_le⟩
  have h_ae : (fun _ : α => τ_base) ≤ᵐ[μ] fun x => ∫⁻ y, K x y ∂μ :=
  ...
  have h_le : τ_base * μ Set.univ ≤ ∫⁻ x, ∫⁻ y, K x y ∂μ ∂μ := by
...
theorem kernel_projection_margin_lower_bound_residual
  ...
  (hμ_fin : μ Set.univ < ⊤) (hμ_pos : 0 < μ Set.univ) :
  ∃ c : ℝ≥0∞, c > 0 ∧
    tau_margin μ Π (Set.univ \ Π '' Set.univ) Set.univ ≥
      c * (∫⁻ x, ∫⁻ y, K x y ∂μ ∂μ) /
        (μ (Set.univ \ Π '' Set.univ) + μ Set.univ + 1) := by
  classical
  set residual := Set.univ \ Π '' Set.univ
  have hA_meas : MeasurableSet residual := hΠ.image_measurable.compl
  ...
  have h_image_subset : Π '' residual ⊆ (Set.univ : Set α) := by intro x hx; trivial
  obtain ⟨c, hc_pos, h_bound⟩ :=
    kernel_projection_margin_lower_bound (layer := layer) (μ := μ) (K := K) (Π := Π)
      hK hΠ (A := residual) (B := Set.univ) hA_meas hB_meas
...
theorem kernel_projection_margin_lower_bound
  ...
  (h_image_subset : Π '' A ⊆ B) :
  ∃ c : ℝ≥0∞, c > 0 ∧
    tau_margin μ Π A B ≥ c * (∫⁻ x, ∫⁻ y, K x y ∂μ ∂μ) /
      (μ A + μ B + 1) := by
  classical
  set residual := Set.univ \ Π '' Set.univ
  have hε_pos : 0 < μ residual :=
    margin_positive_measure (layer := layer) (μ := μ) (K := K) (Π := Π) hK hΠ
  have h_margin_lower :=
    tau_margin_lower_of_residual (layer := layer) (μ := μ) (K := K) (Π := Π)
      hΠ hA hB h_residual_subset h_image_subset
  ...
  have hB_fin' := hB_fin
  obtain ⟨c, hc_pos, h_bound⟩ :=
    kernel_projection_margin_lower_bound_core (layer := layer) (μ := μ) (K := K) (Π := Π)
      hK hΠ hA hB hA_fin' hB_fin'
      (ε := ε) (τ_min := τ_min)
