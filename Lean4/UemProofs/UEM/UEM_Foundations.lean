...
/-- Countable subadditivity of thickness (derived from outer measure laws). -/
theorem thickness_countable_subadditivity (basis : ThicknessBasis O) (s : ℕ → Set O) :
    basis.thickness (⋃ n, s n) ≤ ∑' n, basis.thickness (s n) := by
  exact (thickness_is_outer_measure basis).2.2 s
